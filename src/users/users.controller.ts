...
export class UsersController {
  ...
  @UseGuards(RolesGuard)
  @Roles("User")
  async updateUserAvatar(@Body() body, @Req() req): Promise<IResponse> {
    try {
      var user = await this.usersService.updateAvatar(
        body.avatar,
        req.user.email
      );
      if (user) {
  ...
  @UseGuards(RolesGuard)
  @Roles("User")
  async updatePersonInfo(@Body() body, @Req() req): Promise<IResponse> {
    try {
      var user = await this.usersService.updatePersonInfo(req.user.email, body);
      if (user) {
        return new ResponseSuccess("更新成功", null);
      } else {
        return new ResponseError("更新失败", null);
